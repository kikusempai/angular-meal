...
export class RecipesEffects {
  loadRecipes$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ category, searchQuery, area, ingredient }) => {
        let apiCall;
        ...
        if (searchQuery) {
          ...
          apiCall = this.mealService.filterByArea(area);
        } else if (ingredient) {
          apiCall = this.mealService.filterByIngredient(ingredient);
        } else {
          apiCall = this.mealService.searchMeals('chicken');
        }
    ...
    this.actions$.pipe(
      ...
      switchMap(({ id }) =>
        this.mealService.getMealById(id).pipe(
          map((recipe) => RecipesActions.loadRecipeByIdSuccess({ recipe })),
          catchError((error) =>
            of(
              RecipesActions.loadRecipeByIdFailure({
